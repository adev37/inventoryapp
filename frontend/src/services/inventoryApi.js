...
export const inventoryApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getItems: builder.query({
      ...
      providesTags: (result) =>
        result?.items
          ? [...result.items.map((i) => ({ type: 'Items', id: i._id })), { type: 'Items', id: 'LIST' }]
          : [{ type: 'Items', id: 'LIST' }],
    }),
    ...
    updateItem: builder.mutation({
      ...
      invalidatesTags: (r, e, { id }) => [{ type: 'Items', id }, { type: 'Items', id: 'LIST' }],
    }),
    ...
    getLocations: builder.query({
      ...
      providesTags: (result) =>
        result
          ? [...result.map((l) => ({ type: 'Locations', id: l._id })), { type: 'Locations', id: 'LIST' }]
    ...
    updateLocationByName: builder.mutation({
      query: ({ name, ...body }) => ({ url: `/locations/by-name/${encodeURIComponent(name)}`, method: 'PUT', body }),
      invalidatesTags: [{ type: 'Locations', id: 'LIST' }],
    }),
    ...
    markDemoReturn: builder.mutation({
      ...
      invalidatesTags: [
        { type: 'DemoReturns', id: 'LIST' },
        { type: 'DemoReturns', id: 'REPORT' }, // <-- added so the report refetches
        { type: 'CurrentStock', id: 'LIST' },
        { type: 'Ledger', id: 'LIST' },
      ],
    }),
...
export const {
  ...
  // items
  useGetItemsQuery, useAddItemMutation, useUpdateItemMutation, useDeleteItemMutation,

  // warehouses
  useGetWarehousesQuery, useAddWarehouseMutation, useUpdateWarehouseMutation, useDeleteWarehouseMutation,

  // locations
  useGetLocationsQuery, useAddLocationMutation, useUpdateLocationByNameMutation, useDeleteLocationMutation,

  // stock / ledger / demo / transfers
  useGetCurrentStockQuery,
  useLazyGetCurrentStockQuery,
  useGetCurrentStockSummaryQuery,
  useListStockInQuery, useCreateStockInMutation, useGetStockInChallanQuery,
  useListStockOutQuery, useCreateStockOutMutation, useGetStockOutChallanQuery,
  useGetLedgerQuery,
  useGetPendingDemoReturnsQuery, useMarkDemoReturnMutation, useGetDemoReturnReportQuery,
